...
def read_puzzle_input (filename):
    ordering_rules = {}     # {int : [ints]} = key value : list of values that must precede it (if any of these values follow, the order it bad)
    pages_to_produce = []   # [[ints]] = list of lists of page numbers to check against rules
    section = "top"
        ...
        for line in input_lines:
            if section == "top":
                if len(line) == 0:
                    ...
                    (b,a) = (int(i) for i in line.split('|'))
                    if a in ordering_rules:
                        ordering_rules[a].append(b)
                    else:
                        ordering_rules[a] = [b]
            else:
                pages_to_produce.append([int(i) for i in line.split(',')])
    return (ordering_rules, pages_to_produce)

def check_order (pages, ordering_rules):
    for (idx, p) in enumerate(pages[:-1]):
        if p in ordering_rules:
            for n in pages[idx+1:]:
                if n in ordering_rules[p]:
                    return False
    return True
...
if __name__ == "__main__":
    ...
    (ordering_rules, pages_to_produce) = read_puzzle_input(args.filename)
    final_result = 0

    if verbose:
        print("Ordering rules: if any value in list follows the key value, the order is wrong")
        for r in ordering_rules:
            print(f"   {r}: {ordering_rules[r]}")

    for p in pages_to_produce:
        print(f"checking {p}: ", end='')
        if check_order(p, ordering_rules):
            mid_val = p[(len(p) - 1) // 2]
            print(f"PASS, mid = {mid_val}")
            final_result += mid_val
        else:
            print("fail")

    print(f"Result = {final_result}")

# submitted 6498 = correct
